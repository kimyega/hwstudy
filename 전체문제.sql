...
SELECT USER_NO,
       ...
       USER_REGDATE
  FROM USER_T
 WHERE TO_DATE(USER_REGDATE) >= TO_DATE(2010,'YYYY');
...
SELECT CONCAT(
       ...
       CONCAT(
       USER_NO,' '),
       NVL(USER_MOBILE1, 'NONE')),
       ' '),
       NVL(USER_MOBILE2, 'NONE')) AS 사용자_정보
  FROM USER_T;
...
SELECT U.USER_ID                    AS 아이디,
  ...
  FROM USER_T U, BUY_T B
 WHERE U.USER_NO = B.USER_NO(+)
 GROUP BY U.USER_ID, U.USER_NAME, U.USER_NO
 ORDER BY U.USER_ID;
...
SELECT U.USER_NAME                  AS 고객명,
       P.PROD_NAME                  AS 제품명,
       B.BUY_AMOUNT * P.PROD_PRICE  AS 구매액
...
SELECT P.PROD_NAME                  AS 제품명,
       NVL(COUNT(B.BUY_NO), 0)      AS 판매횟수
   FROM USER_T U, PRODUCT_T P, BUY_T B
 WHERE U.USER_NO(+) = B.USER_NO
   AND B.PROD_CODE(+) = P.PROD_CODE
 GROUP BY P.PROD_NAME, P.PROD_CODE;
...
SELECT U.USER_ID                        AS 아이디,
  ...
  FROM USER_T U, BUY_T B, PRODUCT_T P 
 WHERE U.USER_NO = B.USER_NO
   AND P.PROD_CODE = B.PROD_CODE
 GROUP BY U.USER_ID, U.USER_NAME, U.USER_NO;
