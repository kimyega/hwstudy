...
CREATE TABLE BOOK_T(
    BOOK_ID     NUMBER  (11)         NOT NULL,  -- 책번호
    BOOK_NAME   VARCHAR2(100 BYTE)   NULL,      -- 책이름
    PUBLISHER   VARCHAR2(50 BYTE)    NULL,      -- 출판사명
    PRICE       NUMBER  (6)                     -- 책가격
);

CREATE TABLE CUSTOMER_T(
    CUST_ID     NUMBER  (11)         NOT NULL,  -- 고객 번호 
    CUST_NAME   VARCHAR2(20 BYTE)    NULL,      -- 고객명
    CUST_ADDR   VARCHAR2(50 BYTE)    NULL,      -- 고객 주소
    CUST_TEL    VARCHAR2(20 BYTE)    NULL       -- 고객 전화
);

CREATE TABLE ORDER_T(
   ORDER_ID     VARCHAR2(20 BYTE)    NOT NULL,  -- 주문번호
   CUST_ID      NUMBER  (11)         NULL,      -- 고객번호
   BOOK_ID      NUMBER  (11)         NULL,      -- 책번호
   AMOUNT       NUMBER  (2)          NULL,      -- 판매수량
   ORDER_DT     DATE                 NULL       -- 주문일
);
...
CREATE SEQUENCE BOOK_SEQ ORDER;
...
UPDATE ORDER_T
   SET ORDER_ID = REPLACE(ORDER_DT,'/','') || '-' || ORDER_ID;
...
ALTER TABLE ORDER_T         ADD CONSTRAINT FK_CUSTOMER_ORDER    FOREIGN KEY(CUST_ID)    REFERENCES CUSTOMER_T(CUST_ID);
ALTER TABLE ORDER_T         ADD CONSTRAINT FK_BOOK_ORDER        FOREIGN KEY(BOOK_ID)    REFERENCES BOOK_T(BOOK_ID);    
...
SELECT B.PUBLISHER                  AS 출판사,
       NVL(COUNT(O.ORDER_ID), 0)   AS 판매된책수
  FROM ORDER_T O, BOOK_T B
 WHERE O.BOOK_ID(+) = B.BOOK_ID
...
WITH
    MY_SUBQRY AS (
    SELECT B.PRICE * O.AMOUNT           AS PA,
           COUNT(O.ORDER_ID)            AS CT,
           O.CUST_ID
      FROM ORDER_T O, BOOK_T B
     WHERE O.BOOK_ID = B.BOOK_ID
      GROUP BY O.CUST_ID, B.PRICE * O.AMOUNT
     )
...
SELECT C.CUST_NAME                  AS 고객명,
 ...
 WHERE MY.CUST_ID(+) = C.CUST_ID
 ORDER BY MY.CUST_ID ASC;
